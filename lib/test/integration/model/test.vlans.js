...
describe('AnsiblePOC POST /api/experimental/vlans', function() {
    ...
    it('should return a 400 vlan id is negative', function(done) {
        return client.post('/api/experimental/vlans', {id: -5}, function(err, req, res, obj){
            assert.equal(res.statusCode, 400);
            done();
        });
    });
    it('should return a 400 vlan id is greater than 4096', function(done) {
        return client.post('/api/experimental/vlans', {id: 5000}, function(err, req, res, obj){
            assert.equal(res.statusCode, 400);
            done();
        });
    });
