...
module.exports = function handler(req, res){
    return new Promise(function(resolve, reject) {
        ...
        // TODO Add host_name param, default at nxos-ar130lab-1.yml which must be created inside host_vars folder. (folder must be pre-created)
        ...
        mkdirp(path + config.group_vars_dirname, function(err){
            ...
            mkdirp(path + config.host_vars_dirname, function(err){
                ...
                fs.writeFile(path + config.host_vars_dirname +'/nxos-ar130lab-1.yml', '', function(err){
                    if(err){
                        console.log(err);
                        reject(err);
                    }else{
